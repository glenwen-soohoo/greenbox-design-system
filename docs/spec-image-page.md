...
## 新建頁面前置：先給參考，否則走 Wireframe

形象頁雖有固定鎖定值，但**版面編排**仍需依據。做一個**新建網頁**時：

- **請提供參考網站或參考圖檔**（既有類似頁、設計稿、競品截圖皆可）。
- **若完全沒有參考**，AI 會以 **Wireframe（線框稿）** 的方式製作——只排版面結構與區塊層次、不做視覺完稿；待你補上參考再進一步完稿。

→ 這是為了避免在沒有依據的情況下，AI 自行「發明」不符品牌的版面。

## 字級層級提醒（承核心 [§1.5.1](4-DesignSystem-核心.md)）

- 形象頁會用到 **note（最小級 14/300）** 的場景很明確：**圖片說明文字、footer、版權宣告**；除此之外不要用到 note。
- **內文一律 body**；不可拿 note / small 當內文，也不可拿 body 當標題。
- small 只在特殊情境（表格附註、標籤）使用，不可濫用。
